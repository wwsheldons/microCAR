...
BAT_POW_EN = pyb.Pin(pyb.Pin.cpu.A15, pyb.Pin.OUT_PP)
...
def encrypt_add(dat,key_index):
    ...
    return bytes([i^get_lk(1,key_index)^get_lk(2,key_index) for i in dat])
...
class MicropyLOCK(object):
    ...
    def __init__(self):
        ...
        GL.N_lock = 12 # num of locks
        GL.lock_status=[1]*GL.N_lock
        GL.lose_lock = [10]*GL.N_lock
        ##############
        # hardware port
        self.p = [pyb.Pin(pin_name, pyb.Pin.OUT_PP) for pin_name in POWER_LIST]
        self.can = CAN_PORT
        
        ##############
        # status
        self.ws = [0]*GL.N_lock # banshou status
        self.ls = [0]*GL.N_lock # lock status
        ...
        GL.debug_print('tja1040 init is finished...')
    ...
    def update_lock_status(self):
        for i in range(GL.N_lock):
            if self.ws[i] == 0 or self.ls[i] == 0:
                self.ls[i] = 0
                self.ws[i] = 0
        GL.lock_status = [int(hex(i<<4),16) + j for i,j in zip(self.ws,self.ls)]
    ...
    def send_lock_dat(self):
        ...
        for i in range(len(self.tx_dat)):
            try:
                ...
                continue
        GL.lose_lock[self.tx_dat[0][1]] -= 1
            ...
            if tmpcrc_add == crc_add_frame:
                ...
                if tmp3 == dcrc_add:
                    ...
                    if tmp_id in [i for i in range(GL.N_lock)]:
                        GL.lock_status[tmp_id] = d[1]
                        if GL.init:
                            GL.lcd_update(tmp_id+1)
                        current_ls = d[1] & 0x0f
    ...
    def check_lock(self,lock_id,n = 1):
        for i in range(n):
            if self.check_lock_frame(lock_id,self.random_num):
                if self.send_lock_dat():
                    if  GL.lose_lock[lock_id] <= 0:
                        print('the {}th lock has losed for {}th times'.format(lock_id,i))
                        self.ls[lock_id] = 5
    ...
    def opteate_lock(self,lock_id,order,n = 1):
        ...
        order = 1---open
                2---close
        '''
        frame_type_dict = {2:self.close_lock_frame,1:self.open_lock_frame}
        for i in range(n):
            check_ok = self.check_lock(lock_id)
            if check_ok and frame_type_dict[order](lock_id,self.random_num):
                if self.send_lock_dat():
                    while not self.can.any(0):
                        continue
                    GL.dog.feed()
    ...
    def open_lock(self,lock_id=0):
        if self.ls[lock_id] in [2,3]:
            return self.opteate_lock(lock_id,1)
        GL.debug_print('the {}th lock can not be opend and the self.ls[{}] = {}'.format(lock_id,lock_id,self.ls[lock_id]))
        return 0
    def close_lock(self,lock_id=0):
        if self.ls[lock_id] in [1,3]:
